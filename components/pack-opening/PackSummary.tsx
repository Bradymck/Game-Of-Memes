...
export function PackSummary({ 
  ...
  const generateShareText = (platform: 'twitter' | 'farcaster') => {
    ...
    packs.forEach((packCards, index) => {
      const grid = packCards.map(c => rarityEmojis[c.rarity] || '⬜').join('')
  ...
  const handleShare = async (platform: 'twitter' | 'farcaster' | 'copy') => {
    const shareText = generateShareText(platform === 'copy' ? 'farcaster' : platform)
  ...
  return (
    <div className="fixed inset-0 bg-black/95 flex flex-col items-center justify-center p-4 overflow-y-auto">
        ...
        {cards.map((card) => {
          ...
          return (
            ...
            >
              ...
              <div className="h-1/4 bg-slate-950 p-1">
      ...
      <div className="bg-slate-900/80 rounded-xl p-4 md:p-6 mb-8 max-w-md w-full">
        <h2 className="text-lg font-bold text-white mb-4 text-center">Pack Stats</h2>
          ...
          <div className="border-t border-slate-700 pt-4 mt-4">
            ...
            <div className="flex items-center justify-center gap-3">
              <div 
                ...
                style={{ border: `2px solid ${RARITY_CONFIG[stats.bestCard.rarity].color}` }}
              >
                {stats.bestCard.image ? (
                  <img src={stats.bestCard.image} alt="" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-slate-800 flex items-center justify-center">🎴</div>
                )}
              </div>
              <div>
                <p className="font-bold text-white">{stats.bestCard.name}</p>
                <p 
