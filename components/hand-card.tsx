...
export function HandCard({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (playField && cardRef.current) {
        ...
        const isOverField = cardCenterY < fieldRect.bottom && cardCenterY > fieldRect.top;
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    if (minions && minions.length >= 7) {
      ...
      setTimeout(() => {
        playField?.classList.remove('ring-4', 'ring-red-500', 'ring-opacity-70');
      }, 500);
  ...
  const getTransform = () => {
    ...
    if (isHovered) {
      ...
      return `rotate(${hoverAngle}deg) translateY(${fanOffsetY - 50}px) translateX(${hoverX}px) scale(1.25)` // Much more dramatic!
    }
        ...
        {card.image && card.image !== '/placeholder.jpg' ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-purple-900/40 via-indigo-900/30 to-violet-950/40 relative overflow-hidden">
            <div className="absolute inset-0 opacity-30">
              ...
              <div className="absolute top-1/4 left-1/4 w-16 h-16 rounded-full bg-indigo-400/15 blur-lg animate-pulse" style={{ animationDelay: '0.5s' }} />
            </div>
